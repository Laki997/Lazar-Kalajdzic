...
export class NacionalnostService {
  ...
  dataChange: BehaviorSubject<Nacionalnost[]> = new BehaviorSubject<
    Nacionalnost[]
  >([]);
  ...
  public getAllNacionalnost(): Observable<Nacionalnost[]> {
    this.httpClient.get<Nacionalnost[]>(this.API_URL).subscribe(
      (data) => {
  ...
  public updateNacionalnost(nacionalnost: Nacionalnost): void {
    this.httpClient
      .put(this.API_URL + nacionalnost.id, nacionalnost)
      .subscribe();
  }
